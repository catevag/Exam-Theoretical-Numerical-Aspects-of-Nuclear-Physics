...
def solve_schrodinger(n_grid, dx, v_potential, r2, r, sqrt_r, n, l, i_100):
    ...
    while i  < n_iter and np.absolute(de) > eps:
        ...
        if inv_point < 0 or inv_point >= n_grid - 2:
            print(f"{inv_point:4d} {n_grid:4d} {n}")
        ...
        if (n_cross != nodes):
            ...
            de = df_cusp / ddx12 * y_cusp * y_cusp * dx / 2
            if (de > 0.):
                e_low = e
            if (de < 0.):
                e_up = e
...
def init_potential_effective(r, a, c, d_1):
    ...
    + c * np.power(a, 2) * np.exp(-r / a)
      / (8 * np.pi * np.power(a, 3))
    + d_1 * np.power(a, 4) * np.exp(-r / a)
      / (8 * np.pi * np.power(a, 4) * r) * (r / a - 2)
    )
...
#%%schrodinger renormalization


a = 1 #cutoff

#tuning of the parameter (lowest energy data - phase shift)
c_a2 = -70 # initial guess of the parameter a2

#initial guess of parameters
init_parameters = [-70, -1] #[c, d_1] a4

#cost function for a2-theory
def cost_function_1(c, a, n_grid, dx, r2, r, sqtr_r, n_max):
...
def cost_function_2(parameters, a, n_grid, dx, r2, r, sqtr_r, n_max):
    c = parameters[0]
    d_1 = parameters[1]
    v_potential = init_potential_effective(r, a, c, d_1)
    e, y, phase_shift = solve_schrodinger(n_grid, dx, v_potential, r2, r, sqrt_r, n_max, 0, i_100)
...
#initialize potential
v_potential_3 = init_potential_effective(r, a, c_a2, 0)
v_potential_3_name = "effective_a^2"
v_potential_4 = init_potential_effective(r, a, c_a4, d_1)
...
if a == 1:
    e_eff_a2_1 = e_eff_a2
    e_eff_a4_1 = e_eff_a4
    ph_shift_eff_a2_1 = ph_shift_eff_a2
    ph_shift_eff_a4_1 = ph_shift_eff_a4

if a == 5:
    e_eff_a2_5 = e_eff_a2
    e_eff_a4_5 = e_eff_a4
    ph_shift_eff_a2_5 = ph_shift_eff_a2
    ph_shift_eff_a4_5 = ph_shift_eff_a4

if a == 10:
    e_eff_a2_10 = e_eff_a2
    e_eff_a4_10 = e_eff_a4
    ph_shift_eff_a2_10 = ph_shift_eff_a2
    ph_shift_eff_a4_10 = ph_shift_eff_a4

if a == 0.1:
    e_eff_a2_01 = e_eff_a2
    e_eff_a_01 = e_eff_a4
    ph_shift_eff_a2_01 = ph_shift_eff_a2
    ph_shift_eff_a4_01 = ph_shift_eff_a4
#%%final plots
...
plt.plot(
    np.abs(e_real),
    np.abs(e_real - e_eff_a4_1) / np.abs(e_real),
    marker='o',
    linestyle='--',
...
plt.plot(
    np.abs(e_real),
    np.abs(e_real - e_eff_a2_1) / np.abs(e_real),
    marker='o',
    linestyle='--',
...
plt.plot(
    np.abs(e_real),
    np.abs(ph_shift_real - ph_shift_eff_a4_1) / np.abs(ph_shift_real),
    marker='o',
    linestyle='--',
    label=r'$a^4$ Theory',
    color='orange'
...
#%%a values comparison (run only after collecting data)
# --- Plot 4
#plt.figure()
#plt.plot(
#    np.abs(e_real),
...
#plt.plot(
 #   np.abs(e_real),
 #   np.abs(e_real -  e_eff_a4_5) / np.abs(e_real),
 #   marker='o',
 #   linestyle='--',
 #   label=r'$a=5$',
 #   color='blue'
...
#plt.plot(
 #   np.abs(e_real),
  #  np.abs(e_real -  e_eff_a4_10) / np.abs(e_real),
  #  marker='o',
  #  linestyle='--',
  #  label=r'$a=10$',
  #  color='orange'
...
#plt.xscale("log")
#plt.yscale("log")
#plt.title("Relative Error for Different Values of a")
#plt.legend()
#plt.grid(True)
